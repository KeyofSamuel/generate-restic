Restic Script Generator
=======================

*Version 0.9.6*

The script is functional and has been tested for creating valid backup scripts, but all restic options are not necessarily avaiable in the script generator.  Verify all backup scripts and use at your own risk.

About
+++++

  This is a script generator for restic backups.  It uses a YAML configuration file to set general options and the output is a restic script(s) that can be scheduled with CRON or run manually.  Additionally, it will assist with the creation of a shell script that can be used as an executable to consecutively run backup jobs.  This can also be scheduled with CRON or run manually.

Creating Backup Scripts
-----------------------

  Once the application is installed *(see below)*, backup scripts are created with a two-step procedure.  First, edit the contents of the 'config.yml' file with backup job names and options.  Detailed instructions for the configuration file options are below.  Secondly, run the 'generate_restic' script and choose option #1 to create backup job scripts for each job listed in the 'config.yml' file.  You can then select option #2 to create a script that will run all of the desired backup jobs consecutively.  To run the backups, manually run each file (or the consecutive scripts) or schedule the backup scripts (or the consecutive scripts) with CRON or a systemd timer.
...
++++++++++++++++++++++++++++++
  ...
  The application is configured to look for a YAML configuration file in '~/.config/restic-backup/' called 'config.yml'.  In the Git repository is a 'config.yml.sample' file that provides a template for how to set up backups and configure the options.  Setting up the config file is covered in the section above.
